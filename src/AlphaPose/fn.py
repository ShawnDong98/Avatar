...
def draw_single(frame, pts, joint_format='coco'):
    ...
    for i, (start_p, end_p) in enumerate(l_pair):
        # part_line 是一个字典， 存储 关节点为key 坐标为value
        if start_p in part_line and end_p in part_line:
            start_xy = part_line[start_p]
            end_xy = part_line[end_p]
            cv2.line(frame, start_xy, end_xy, line_color[i], int(2))
    return frame, pts


def draw_single_test(frame, pts, joint_format='coco'):
    if joint_format == 'coco':
        l_pair = COCO_PAIR
        p_color = POINT_COLORS
        line_color = LINE_COLORS
    ...
    # 14个特征点？ 
    pts = np.concatenate((pts, np.expand_dims((pts[1, :] + pts[2, :]) / 2, 0)), axis=0)
    np.savetxt('test.txt', pts)
    for n in range(pts.shape[0]):
        # 第三个维度 表示正确率
        # if pts[n, 2] <= 0.05:
        #     continue
        # 前两个维度表示坐标
        cor_x, cor_y = int(pts[1, 0]), int(pts[1, 1])
        part_line[1] = (cor_x, cor_y)
        cv2.circle(frame, (cor_x, cor_y), 3, p_color[1], -1)

        cor_x, cor_y = int(pts[3, 0]), int(pts[3, 1])
        part_line[3] = (cor_x, cor_y)
        cv2.circle(frame, (cor_x, cor_y), 3, p_color[3], -1)

    # 根据pair关系画线
    for i, (start_p, end_p) in enumerate(l_pair):
        # part_line 是一个字典， 存储 关节点为key 坐标为value
        if start_p in part_line and end_p in part_line:
            start_xy = part_line[1]
            end_xy = part_line[3]
            cv2.line(frame, start_xy, end_xy, line_color[0], int(1*(pts[1, 2] + pts[3, 2]) + 1))
    return frame, pts
