...
def vis_label(frame, im_res, format='coco'):
    '''
    frame: frame image
    im_res: im_res of predictions
    format: coco or mpii
    ...
    im_name = im_res['imgname'].split('/')[-1]
    img = np.zeros((frame.shape[:2]+(1, ))).astype(np.uint8)
    height,width = img.shape[:2]

    #--- 先缩小分辨率画图，画好后插值上采样 ---
    img = cv2.resize(img,(int(width/2), int(height/2)))
    pts_list = []
    if len(im_res['result']) > 2:
            im_res['result'] = im_res['result'][:2]
    for human in im_res['result']:
        #--- 如果人数多于两个不要 ---
        part_line = {}
        kp_preds = human['keypoints']
        ...
        kp_preds = process_kps(kp_preds).numpy()
        pts_list.append(kp_preds)
