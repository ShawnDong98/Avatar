...
class WebcamDetectionLoader:
    def __init__(self, webcam = 0, batchSize=1, queueSize=1256):
        # initialize the file video stream along with the boolean
        # used to indicate if the thread should be stopped or not
        self.det_model = Darknet("yolo/cfg/yolov3-spp.cfg")
        self.det_model.load_weights('models/yolo/yolov3-spp.weights')
            ...
            if boxes is None:
                ...
                if opt.save_img or opt.save_video or opt.vis:
                    ...
                    if opt.vis:
                        ...
                        # if not os.path.exists(f"./output/{opt.video.split('.')[-2]}/{opt.video.split('.')[-2]}_img/"):
                        #     os.makedirs(f"./output/{opt.video.split('.')[-2]}/{opt.video.split('.')[-2]}_img/")
                        # cv2.imwrite(f"./output/{opt.video.split('.')[-2]}/{opt.video.split('.')[-2]}_img/{im_name.split('.')[-2]}.png", img)
                        


                        cv2.imshow("AlphaPose label", label)
                        # if not os.path.exists(f"./output/{opt.video.split('.')[-2]}/{opt.video.split('.')[-2]}_label/"):
                        #     os.makedirs(f"./output/{opt.video.split('.')[-2]}/{opt.video.split('.')[-2]}_label/")
                        # cv2.imwrite(f"./output/{opt.video.split('.')[-2]}/{opt.video.split('.')[-2]}_label/{im_name.split('.')[-2]}.png", label)
